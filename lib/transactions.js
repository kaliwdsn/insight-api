...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  var self = this;
  Request('http://bws.gat.hoopox.com/bws/api/v1/tx/' + transaction.hash, function(err, response, body) {
      if (!err) {
        try {
...
TxController.prototype.transformOutput = function(options, output, index) {
  ...
  if (options.extra && !transformed.scriptPubKey.type){
    transformed.scriptPubKey.customData = options.extra.customData;
  }
  return transformed;
};
