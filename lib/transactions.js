...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  getColoredTx(transaction.hash, function (err, cctx) {
    if (err) {
      return callback(err);
    }
    ...
    var transformed = {
      ...
      cctx: cctx,
      ccdata: cctx.ccdata,
      ccparsed : cctx.ccparsed || false,
      colored : cctx.colored || false,
    };
...
TxController.prototype.transformInput = function(options, input, index) {
  ...
  if (options.cctx && options.cctx.vin && options.cctx.vin[index]) {
    transformed.assets = options.cctx.vin[index].assets || [];
  }
...
TxController.prototype.transformOutput = function(options, output, index) {
  ...
  if (options.cctx && options.cctx.vout && options.cctx.vout[index]) {
    transformed.assets = options.cctx.vout[index].assets || [];
  }
...
function getColoredTx(txid, callback) {
  Request('http://cexp.gat.hoopox.com/api/gettransaction?txid=' + txid, function(err, response, body) {
    if (err) {
      return callback(err);
    }
