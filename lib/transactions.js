...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  async.map(wrappedOutputs, function(item, cb) {
      ...
      if (output.cctx) {  
        var cctx = output.cctx;
        var type = cctx.type;
        output.type = type;
        if (type === 'issuance') {
          var address = transaction.outputs[0].address;
          if (address) {
            return getAssetByAddress(address, function(err, metadata) {
              output.metadata = metadata || null;
              cctx.meta = metadata;
              transformed.cctx = cctx;
              cb(err, output);
            });
          } 
        } else if (type === 'transfer'){
          output.fromAddress = transaction.inputs[0].address;
          output.toAddress = transaction.outputs[0].address;

          if (output.toAddress) {
            return getAssetByAddress(output.toAddress, function(err, metadata) {
              output.metadata = metadata || null;
              cctx.meta = metadata;
              cctx.fromAddress = output.fromAddress;
              cctx.toAddress = output.toAddress;
              transformed.cctx = cctx;
              cb(err, output);
            });
          } 
...
TxController.prototype.transformOutput = function(options, output, index) {
  ...
  if (output.address) {
    ...
    var asm = output.scriptAsm;
    if (asm.indexOf('OP_RETURN') === 0) {
      var op_return = asm.split(' ')[1];
      var txInfo = CCTransaction.fromHex(op_return);
      transformed.cctx = txInfo;
    }
  }
