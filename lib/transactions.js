...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  async.map(wrappedOutputs, function(item, cb) {
      ...
      if (output.cctx) {
        ...
        } else if (type === 'transfer' && item.index > 1){
          output.fromAddress = transaction.outputs[item.index - 1].address;
          output.toAddress = transaction.outputs[item.index - 2].address;
